...
// Serial command input for Arduino Nano. A command is executed only after
// CR or LF is received. Fixed-size storage avoids String heap fragmentation.
const byte commandLineCapacity = 64;
char commandLine[commandLineCapacity];
byte commandLineLength = 0;
bool commandLineOverflow = false;
...
void setup() {
	Serial.begin(115200); // Arduino Nano USB serial
...
void loop() {
	while (Serial.available() > 0) {
		...
		if (incoming == '\n' || incoming == '\r') {
			if (commandLineOverflow) {
				Serial.println("Command too long, line discarded.");
				commandLineOverflow = false;
				commandLineLength = 0;
				Serial.println("Ready.");
				continue;
			}
		...
		if (commandLineOverflow) {
			continue; // Discard characters until the line terminator.
		}

		if (commandLineLength < commandLineCapacity - 1) {
			commandLine[commandLineLength++] = incoming;
		}
		else {
			commandLineOverflow = true;
		}
	}
}
...
void processCommandLine(char* line) {
	...
	if (command != 's' && command != 'r') {
		if (*cursor == '\0') {
			Serial.println("Missing command parameter.");
			return;
		}

		parameter = (byte)*cursor++;
		if (parameter >= 'A' && parameter <= 'Z') {
			parameter = parameter - 'A' + 'a';
		}
	}
	else if (command == 'r' && *cursor != '\0') {
		// Reset accepts optional 'f': rf forces reset while BMC is busy.
		parameter = (byte)*cursor++;
		if (parameter >= 'A' && parameter <= 'Z') {
			parameter = parameter - 'A' + 'a';
		}
	...
	commandArguments = cursor;

	if (command == 'i') {
		if (parameter < '0' || parameter > '2') {
			Serial.println("Invalid init mode. Use i0, i1 or i2.");
		}
		else {
		...
		if (parameter != 0 && parameter != 'f') {
			Serial.println("Invalid reset command. Use r or rf.");
		}
		else {
		...
		if (parameter != 'r' && parameter != 'w') {
			Serial.println("Invalid FIFO command. Use fr or fw.");
		}
		else {
...
long processPageNumber(long startingPage) {
	...
	// Skip separators before the next decimal number.
	while (*commandArguments == ' ' || *commandArguments == '\t' ||
	       *commandArguments == ',' || *commandArguments == ':') {
		commandArguments++;
	}

	if (*commandArguments < '0' || *commandArguments > '9') {
		return startingPage;
	}

	long page = 0;
	while (*commandArguments >= '0' && *commandArguments <= '9') {
		page = page * 10 + (*commandArguments - '0');
		commandArguments++;
	}
	...
	// If invalid, return the supplied default/start page.
	return startingPage;
}
...
void processFifo(byte frwByte) {
	...
	if (frwByte == 'r') {
		...
		// With line-based input, bytes are written as hex tokens:
		// fw FF 00 A5 7E
		while (commandArguments != NULL && *commandArguments != '\0' && count < 43) {
			while (*commandArguments == ' ' || *commandArguments == '\t' ||
			       *commandArguments == ',' || *commandArguments == ':') {
				commandArguments++;
			}
			...
			if (commandArguments[0] == '0' &&
			    (commandArguments[1] == 'x' || commandArguments[1] == 'X')) {
				commandArguments += 2;
			}
			...
			if (digits == 0) {
				Serial.println("Invalid hex byte in FIFO data.");
				break;
			}

			rwBuffer[count++] = value;
		}
